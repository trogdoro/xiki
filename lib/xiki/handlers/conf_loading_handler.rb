module Xiki
  class ConfLoadingHandler
    def self.handle options
      ...
      # Always try loading options[:conf] (from /index.conf or ~/xiki/roots/conf/foo.conf...
    ...
    # Populates options[:conf] based on /index.conf or ~/xiki/roots/conf/foo.conf.
    def self.load_conf options

      # Load user conf, if there is any...

      user_conf = File.expand_path "~/.xiki/roots/conf/#{options[:name]}.conf"
